...
public sealed class MeleeWindupOverlay : Overlay
{
    ...
    public MeleeWindupOverlay(IEntityManager entManager, IGameTiming timing, IPlayerManager playerManager, IPrototypeManager protoManager)
    {
        ...
        var sprite = new SpriteSpecifier.Rsi(new ("/Textures/_Crescent/Objects/Fun/Plushies/pearcat.rsi"), "icon"); // SPRITESTATE REPLACED
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        //handle.DrawTexture(_texture, position); COMMENTED OUT FOR TESTS
        ...
        //var releaseWidth = 2f * 0.2f / (float) _melee.GetWindupTime(meleeUid, owner.Value, comp).TotalSeconds * EyeManager.PixelsPerMeter;
        //const float releaseSlash = startX + 4f;
        //const float releaseSuperSlash = releaseSlash + 6f;

        //var releaseSlashBox = new Box2(new Vector2(releaseSlash - releaseWidth / 2f, 3f) / EyeManager.PixelsPerMeter,
        //    new Vector2(releaseSlash + releaseWidth / 2f, 4f) / EyeManager.PixelsPerMeter);

        //var releaseSuperSlashBox = new Box2(new Vector2(releaseSuperSlash - releaseWidth / 2f, 3f) / EyeManager.PixelsPerMeter,
        //    new Vector2(releaseSuperSlash + releaseWidth / 2f, 4f) / EyeManager.PixelsPerMeter);

        //releaseSlashBox = releaseSlashBox.Translated(position); COMMENTED OUT FOR TESTS
        //handle.DrawRect(releaseSlashBox, Color.LimeGreen);

        //releaseSuperSlashBox = releaseSuperSlashBox.Translated(position);
        //handle.DrawRect(releaseSuperSlashBox, Color.Red);
        ...
        ////handle.DrawRect(box, Color.White); COMMENTED OUT FOR TESTS

        if (comp.Snapshot == null)
        {
